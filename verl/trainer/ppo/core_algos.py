...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        all_means = torch.tensor(list(id2mean.values()))
        overall_mean = torch.mean(all_means)
        ...
        for i in range(bsz):
            ...
            if grpo_weight == 2.0:
                scores_difffouces = 0.5 * scores[i] / (torch.sqrt(id2mean[index[i]]) + epsilon)
                scores_easyfouces = 0.5 * scores[i] / (1-torch.sqrt(id2mean[index[i]]) + epsilon)
                scores[i] = (1-overall_mean) * scores_easyfouces + overall_mean * scores_difffouces
                if scores[i] > 0:
                    alpha = 1 #0.5 for geo3k and med, 1 for  math
                    wieght = min(1, overall_mean/alpha)
                    scores[i] = scores[i] * wieght
...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange, token_level_scores, 
    ...
    if positive_learning_weight is not None:
        ...
        pos_pg_losses = pg_losses1[correct_idx]
        ...
        neg_pg_losses = pg_losses1[incorrect_idx]
