...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, advantage='positive', positive_advantage_weight=1, grpo_weight = 1, norm_adv_by_std_in_grpo=True):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        correct_idx = batch.batch['token_level_scores'].sum(-1) >= 0.9
                        incorrect_idx = batch.batch['token_level_scores'].sum(-1) <= 0.1
                        ...
                        grpo_weight = self.config.algorithm.grpo_weight + (self.global_steps/self.total_training_steps)*(1-self.config.algorithm.grpo_weight)
                        ...
                        batch = compute_advantage(batch,
                                                  ...
                                                  #grpo_weight=self.config.algorithm.grpo_weight,
                                                  grpo_weight=grpo_weight,
                                                  )
                    # update critic
                    if self.use_critic:
